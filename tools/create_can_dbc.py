...
for axisID in range(0, 8):
    ...
    axisError.choices={error.value: error.name for error in AxisError}
    axisState = can.Signal("Axis_State", 32, 8, receivers=['Master'])
    axisState.choices={state.value: state.name for state in AxisState}
    ...
    motorError.choices={error.value: error.name for error in MotorError}
    ...
    encoderError.choices={error.value: error.name for error in EncoderError}
    ...
    sensorlessError.choices={error.value: error.name for error in SensorlessEstimatorError}
    ...
    axisRequestedState.choices={state.value: state.name for state in AxisState}
    ...
    encoderPosEstimate = can.Signal("Pos_Estimate", 0, 32, receivers=['Master'], unit='rev')
    encoderPosEstimate.is_float=True
    encoderVelEstimate = can.Signal("Vel_Estimate", 32, 32, receivers=['Master'], unit='rev/s')
    ...
    ControlMode.choices={state.value: state.name for state in ControlMode}
    inputMode = can.Signal("Input_Mode", 32, 32, receivers=[newNode.name])
    inputMode.choices={state.value: state.name for state in InputMode}
    setControllerModeMsg = can.Message(
        0x00B, "Set_Controller_Mode", 8, [controlMode, inputMode], senders=['Master']
    )

    # 0x00C - Set Input Pos
    inputPos = can.Signal("Input_Pos", 0, 32, receivers=[newNode.name], unit='rev')
    inputPos.is_float=True
    velFF = can.Signal("Vel_FF", 32, 16, is_signed=True, receivers=[newNode.name], unit='rev/s')
    velFF.scale=0.001
    torqueFF = can.Signal("Torque_FF", 48, 16, is_signed=True, receivers=[newNode.name], unit='Nm')
    ...
    # 0x00D - Set Input Vel
    inputVel = can.Signal("Input_Vel", 0, 32, receivers=[newNode.name], unit='rev')
    inputVel.is_float=True
    inputTorqueFF = can.Signal("Input_Torque_FF", 32, 32, receivers=[newNode.name], unit='rev/s')
    ...
    inputTorque = can.Signal("Input_Torque", 0, 32, receivers=[newNode.name], unit='Nm')
    ...
    velLimit = can.Signal("Velocity_Limit", 0, 32, receivers=[newNode.name], unit='rev/s')
    velLimit.is_float=True
    currentLimit = can.Signal("Current_Limit", 32, 32, receivers=[newNode.name], unit='A')
    ...
    trajVelLim = can.Signal("Traj_Vel_Limit", 0, 32, receivers=[newNode.name], unit='rev/s')
    ...
    trajAccelLim = can.Signal("Traj_Accel_Limit", 0, 32, receivers=[newNode.name], unit='rev/s^2')
    trajAccelLim.is_float=True
    trajDecelLim = can.Signal("Traj_Decel_Limit", 32, 32, receivers=[newNode.name], unit='rev/s^2')
    ...
    trajInertia = can.Signal("Traj_Inertia", 0, 32, receivers=[newNode.name], unit='Nm / (rev/s^2)')
    ...
    # 0x014 - Get Iq
    iqSetpoint = can.Signal("Iq_Setpoint", 0, 32, receivers=['Master'], unit='A')
    iqSetpoint.is_float=True
    iqMeasured = can.Signal("Iq_Measured", 32, 32, receivers=['Master'], unit='A')
    ...
    sensorlessPosEstimate = can.Signal("Sensorless_Pos_Estimate", 0, 32, receivers=['Master'], unit='rev')
    sensorlessPosEstimate.is_float=True
    sensorlessVelEstimate = can.Signal("Sensorless_Vel_Estimate", 32, 32, receivers=['Master'], unit='rev/s')
    sensorlessVelEstimate.is_float=True
    ...
    busVoltage = can.Signal("Bus_Voltage", 0, 32, receivers=['Master'], unit='V')
    busVoltage.is_float=True
    busCurrent = can.Signal("Bus_Current", 32, 32, receivers=['Master'], unit='A')
    busCurrent.is_float=True
    ...
    # 0x01A - Set Pos gain
    posGain = can.Signal("Pos_Gain", 0, 32, receivers=[newNode.name], unit='(rev/s) / rev')
    posGain.is_float=True
    setPosGainMsg = can.Message(0x01A, "Set_Pos_Gain", 8, [posGain], senders=['Master'])

    # 0x01B - Set Vel Gains
    velGain = can.Signal("Vel_Gain", 0, 32, receivers=[newNode.name], unit='Nm / (rev/s)')
    velGain.is_float=True
    velIntGain = can.Signal("Vel_Integrator_Gain", 32, 32, receivers=[newNode.name], unit='(Nm / (rev/s)) / s')
    ...
    adcVoltage = can.Signal("ADC_Voltage", 0, 32, receivers=['Master'], unit='V')
    adcVoltage.is_float=True
    ...
    controllerError.choices={error.value: error.name for error in ControllerError}
